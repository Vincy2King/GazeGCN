...
# You may obtain a copy of the License atoutputs.shape
#
...
class EyeEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.revise_gat!='org':
            self.eye_embeddings = nn.Embedding(config.eye_length[config.revise_gat], config.hidden_size, padding_idx=config.pad_token_id)
