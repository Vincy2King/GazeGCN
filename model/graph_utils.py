...
class GraphAttentionHead(nn.Module):
    ...
    def forward(self, h, adj):
        ...
        # 论文的公式 3
        # ​torch.where​​这是一个新的函数。
        # 在使用A[A>x] = 1这样的in-place操作是不可导的，
        # 所以要使用torch.where（condiciton，B，A）函数。满足条件的A会被对应位置的B替代。
        # 所以代码中就是，zero_vec的邻接矩阵大于0的位置的值会被替换成刚刚计算出来的e的对应位置的值。
        # 这个就是atteniton，表示临界的节点对于这个节点的不同的重要性的概念吧。
...
class GATModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.eye_max_syntactic_distance = config.eye_max_syntactic_distance
        self.revise_gat = config.revise_gat
        self.revise_edge = config.revise_edge
        self.use_eye = config.use_eye
        ...
        # print('config.pos_tag_vocab_size:', config.pos_tag_vocab_size)
        # print('self.pos_tag_embed:', self.pos_tag_embed)
        self.graph_attention = []
        gat_out_size = head_dim * config.num_gat_head
        # print('gat_out_size:',gat_out_size)
        ...
        # feed text graph representation into gaze graph
        self.gaze_graph_attention = []
        # gat_out_size = head_dim * config.num_gat_head
        # print('gat_out_size:', gat_out_size)
        for i in range(config.num_gat_layer):
            input_size = gat_out_size
            self.gaze_graph_attention.append(
                GraphAttentionNetwork(
                    nfeat=input_size,
                    nhid=head_dim,
                    dropout=config.hidden_dropout_prob,  # 0.1
        ...
        self.gaze_graph_attention = nn.ModuleList(self.gaze_graph_attention)
        self.gaze_graph_attention_layer_norm = nn.LayerNorm(gat_out_size, eps=config.layer_norm_eps)
        ...
        '''
    def forward(
            self, inputs_embeds, dist_mat,eye_dist_mat, deptag_ids=None, postag_ids=None,
            FFDs=None,
            GDs=None,
            GPTs=None,
            TRTs=None,
            ...
            is_threhold=True,
            is_text_gat=False,
            is_eye_threhold=False,
    ):
        gat_rep = inputs_embeds
        if is_text_gat ==False:
            print('gat_rep shape:', gat_rep.shape)
            # print('FFD shape:',FFDs.shape)
            # print('deptag_ids shape:',deptag_ids.shape)
            if self.use_eye == 'org':
                if self.dep_tag_embed is not None:
                    ...
                    gat_rep = gat_rep + dep_embeddings
                    # print('gat_rep shape:', gat_rep.shape)
                    # print('dep_embeddings shape:',dep_embeddings.shape)
                if self.pos_tag_embed is not None:
                    ...
                    print('postag_ids:',postag_ids.shape)
                    print('pos_embeddings shape:', pos_embeddings.shape)
            elif self.use_eye == 'eye':
                if self.dep_tag_embed is not None:
                    assert deptag_ids is not None
                    dep_embeddings = self.dep_tag_embed(deptag_ids)  # B x T x (L*head_dim)
                ...
                if self.pos_tag_embed is not None:
                    ...
                    pos_embeddings = self.pos_tag_embed(postag_ids)  # B x T x (L*head_dim)
                    gat_rep += pos_embeddings
            elif self.use_eye == 'fuse':
                if FFDs is not None:
                    assert FFDs is not None
                    dep_embeddings = self.dep_tag_embed(deptag_ids)  # B x T x (L*head_dim)
                    gat_rep += dep_embeddings
                if self.pos_tag_embed is not None:
                    assert postag_ids is not None
        ...
        eye_adj_mat = eye_dist_mat.clone()
        if is_threhold == True:
            adj_mat[dist_mat <= self.max_syntactic_distance] = 1
            adj_mat[dist_mat > self.max_syntactic_distance] = 0
        if is_eye_threhold == True:
            eye_adj_mat[eye_dist_mat <= self.eye_max_syntactic_distance] = 1
            eye_adj_mat[eye_dist_mat > self.eye_max_syntactic_distance] = 0
        # print('======adj_mat=======')
        # print(adj_mat)
        if self.revise_edge == 'fuse_dist':
            adj_mat = adj_mat + eye_dist_mat
        # print(eye_dist_mat)
        # print(adj_mat)
        if is_text_gat == False:
            ...
            for _, gatlayer in enumerate(self.gaze_graph_attention):
                gat_rep = gatlayer(gat_rep, adj_mat)
                print('========gaze_gat_rep========')
                print(gat_rep.shape)
